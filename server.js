 ...
 * Residencial API
 ****************************************************************/
var Residencial = mongoose.model("Residencial", {
    nombre: String,
    direccion: String,
    telefono: String
});

app.get('/api/residencial', function(req, res) {
    Residencial.find(function(err, residenciales) {
        if(err) {
            res.send(err);
        }
        res.json(residenciales);
    });
});

app.post('/api/residencial', function(req, res) {
    Residencial.create({
        nombre: req.body.nombre,
        direccion: req.body.direccion,
        telefono: req.body.telefono
    }, function(err, residencial){
        if(err) {
            res.send(err);
        }
...
app.delete('/api/residencial/:id', function(req, res) {
    Residencial.remove({
        _id: req.params.id
    }, function(err, residencial) {
        if(err){
